...
class PeleeNet:
    ...
    def peleenet(self, input_x, k=32, num_init_channel=64, block_config=[3,4,4,3], bottleneck_width=[2,2,4,4], is_training=True, reuse=False):
        with tf.variable_scope(self.model_name) as scope:
            ...
            for num_block, bottleneck_coeff in zip(block_config, bottleneck_width):
                ...
                if stage == 2:
                    pre_scale2 = from_layer
                if stage == 3:
                    pre_scale3 = from_layer
                if stage == 4:
                    from_layer = self.layer.get_layer2x1(from_layer, pre_scale3, training=is_training)
                    from_layer = self.layer.get_layer2x2(from_layer, pre_scale2, training=is_training)
                    # from_layer = self.layer.get_layer2x3(from_layer, pre_scale1)
                    from_layer = self.layer.get_layer_conv(from_layer, training=is_training)
